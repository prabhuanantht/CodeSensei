...
def monkey_patch_node_for_logging(
    node, step_name: str, logger: StreamlitProgressLogger
):
    """
...
def monkey_patch_batch_node_for_logging(
    node, step_name: str, logger: StreamlitProgressLogger
):
    """
...
def create_instrumented_flow(logger: StreamlitProgressLogger):
    ...
    steps = [
        ...
        (
            flow.start.next_nodes[0].next_nodes[0].next_nodes[0].next_nodes[0],
            "Write Chapters",
            True,
        ),
        (
            flow.start.next_nodes[0]
            .next_nodes[0]
            .next_nodes[0]
            .next_nodes[0]
            .next_nodes[0],
...
def validate_config(config: dict) -> tuple[bool, str]:
    ...
    if not config.get("repo_url") and not config.get("local_dir"):
        return (
            False,
            "Please provide either a GitHub repository URL or a local directory path",
        )
    ...
    if config.get("repo_url") and config.get("local_dir"):
        return (
            False,
            "Please provide only one source: either GitHub URL or local directory",
        )
