...
class CodeIntelligenceAnalyzer:
    ...
    def _analyze_complexity(self, files_data: List[Tuple[str, str]]) -> Dict[str, Any]:
        ...
        if self.function_metrics:
            avg_cc = sum(f['cyclomatic_complexity'] for f in self.function_metrics) / len(self.function_metrics)
            complex_funcs = sum(1 for f in self.function_metrics if f['cyclomatic_complexity'] > 10)
        else:
            avg_cc = 0
            complex_funcs = 0
    ...
    def _analyze_file_calls(self, filepath: str, content: str):
        ...
        for node in ast.walk(tree):
            if isinstance(node, ast.FunctionDef):
                ...
                if self.graph:
                    self.graph.add_node(full_name, type="function", file=filepath, 
                ...
                self.all_names.add(node.name)
                if self.graph:
                    self.graph.add_node(full_name, type="class", file=filepath, 
                                       line=node.lineno, name=node.name)
    ...
    def _analyze_patterns(self, files_data: List[Tuple[str, str]]) -> Dict[str, Any]:
        ...
        # Count patterns
        func_patterns = [p for p in self.all_patterns if p.get("type") == "function_structure"]
        ...
        meaningful_patterns = [(p, c) for p, c in pattern_counts.items() 
        ...
        # Get class patterns
        class_patterns = [p for p in self.all_patterns if p.get("type") == "class_structure"]
        ...
        return {
            ...
            'class_stats': {
                'avg_methods': sum(p['method_count'] for p in class_patterns) / max(len(class_patterns), 1) if class_patterns else 0,
    ...
    def _classify_pattern(self, pattern: List[str]) -> str:
        ...
        if any(token.startswith("CALL:") and "download" in token.lower() for token in pattern):
            return "Web Scraping"
        if any(token.startswith("CALL:") and "regex" in token.lower() for token in pattern):
    ...
    def _detect_anti_patterns(self, func_patterns: List[Dict], files_data: List[Tuple[str, str]]) -> List[Dict]:
        ...
        for pattern_info in func_patterns:
            ...
            # Missing return
            if "RETURN" not in pattern and not any(p.startswith("CALL:") for p in pattern):
                anti_patterns.append({
                    "type": "MISSING_RETURN",
                    "file": filepath,
                    "line": line,
            ...
            try:
                ...
                for node in ast.walk(tree):
                    if isinstance(node, ast.FunctionDef):
                        ...
                        if func_code and len(func_code.strip()) > 20:  # Skip very short functions
                            # Get embedding
                            try:
                                embedding = self._get_code_embedding(func_code, tokenizer, model)
                                embeddings[func_name] = embedding
                                function_code[func_name] = func_code[:200]  # First 200 chars
                            except Exception as e:
                                print(f"  Error getting embedding for {func_name}: {str(e)[:50]}")
                                continue
